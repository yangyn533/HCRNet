...
class GlobalExpectationPooling1D(K.layers.Layer):
    ...
    def call(self, x, **kwargs):
        if self.mode == 0:
            ...
            # x = x - max(x)
            diff_1 = tf.subtract(now, tf.reduce_max(now, axis=-1, keepdims=True))
            # x = mx
            diff = tf.multiply(diff_1, self.m)
            # prob =  exp(x_i)/sum(exp(x_j))
            prob = tf.nn.softmax(diff)
            # Expectation = sum(Prob*x)
            expectation = tf.reduce_sum(tf.multiply(now, prob), axis=-1, keepdims=False)
        else:
            # transform the input
            now = tf.transpose(x, [0, 2, 1])
            # x  - mean(x)
            now_diff = tf.subtract(now, tf.reduce_mean(now, axis=-1, keepdims=True))
            # x = mx
            ...
            prob = diff_now / tf.reduce_sum(diff_now, axis=-1, keepdims=True)
            expectation = tf.reduce_sum(tf.multiply(now, prob), axis=-1, keepdims=False)
        return expectation
